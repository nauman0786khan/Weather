...
console.log(rr);
let temp = `<h2>${(rr.main.temp-273.15).toFixed(2)}</h2><p>feels like ${(rr.main.feels_like-273.15).toFixed(2)}</p>`
let name = `<h2>${rr.name}</h2>`
let humidity = `<h2>${rr.main.humidity}</h2>`
let pressure = `<h2>${rr.main.pressure}</h2>`
let wind = `<h2>deg ${rr.wind.deg} Speed ${(rr.wind.speed*3.6).toFixed(2)}Km/h</h2>`
...
if(wc=="Clouds"){
    ...
    document.getElementById("vidc").src="Video/production_id_4687100 (2160p).mp4";
